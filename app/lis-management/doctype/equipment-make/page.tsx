...
export default function DoctypePage() {
  ...
  // 🟢 Use filtered makes for display but original makes for pagination count
  const displayMakes = filteredMakes;
  ...
  const {
    ...
    clearSelection,
    isAllSelected
  } = useSelection(displayMakes, "name");

  const [isDeleting, setIsDeleting] = React.useState(false);

  // ── 🟢 Fetch Logic (Refactored for Pagination and Total Count) ───────────────────
  const fetchMakes = React.useCallback(
      ...
      try {
        ...
        const [dataResp, countResp] = await Promise.all([
          axios.get(`${API_BASE_URL}/api/resource/${doctypeName}`, {
            params: {
              fields: JSON.stringify(["name", "equipement_make", "modified"]),
              limit_start: start,
        ...
        const mapped: EquipementMake[] = raw.map((r: any) => ({
          name: r.name,
          equipement_make: r.equipement_make ?? r.name,
          modified: r.modified,
        }));
        ...
        if (isReset) setError(err.response?.status === 403 ? "Unauthorized" : "Failed to fetch equipment makes");
      } finally {
        setLoading(false);
            ...
            <th className="text-right pr-4" style={{ width: "120px" }}>
              <div className="flex items-center justify-end gap-1 text-[10px] font-medium text-gray-500 uppercase tracking-wider">
                 {loading ? <Loader2 className="w-3 h-3 animate-spin" /> : (
                   <><span>{displayMakes.length}</span><span className="opacity-50"> /</span><span className="text-gray-900 dark:text-gray-200 font-bold">{totalCount}</span></>
                 )}
              </div>
            </th>
          </tr>
