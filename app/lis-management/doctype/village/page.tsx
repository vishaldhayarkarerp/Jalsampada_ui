...
export default function VillageListPage() {
  ...
  const fetchData = React.useCallback(async () => {
    ...
    try {
      ...
      const params: any = {
        fields: JSON.stringify(["name", "village", "taluka", "district", "modified"]),
      ...
      if (searchTerm) {
        params.filters = JSON.stringify([
          ["Village", "village", "like", `%${searchTerm}%`]
        ]);
      }
          ...
          {rows.length ? (
            rows.map((row) => (
              <tr key={row.name} onClick={() => handleCardClick(row.name)} className="hover:bg-muted/50 cursor-pointer">
                ...
                <td className="p-3">{row.taluka}</td>
      ...
      {rows.length ? (
        rows.map((row) => (
          <RecordCard
            key={row.name}
            title={row.village}
            subtitle={`${row.taluka}, ${row.district}`}
            fields={[{ label: "ID", value: row.name }]}
            onClick={() => handleCardClick(row.name)}
          />
        ))
      ) : (
        <p className="col-span-full text-center py-8 text-muted-foreground">No villages found.</p>
      )}
    </div>
  );
