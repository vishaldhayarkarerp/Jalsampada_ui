...
/* ─────────────────────────────────────────────── */
export default function DeviceTypeListPage() {
  ...
  const filteredRecords = React.useMemo(() => {
    ...
    return records.filter(
      (r) =>
        ...
        r.device_type?.toLowerCase().includes(debouncedSearch.toLowerCase())
    );
  ...
  const handleCardClick = (id: string) => {
    router.push(`/maintenance/doctype/device-type/${encodeURIComponent(id)}`);
  };
  ...
  const currentSortLabel =
    SORT_OPTIONS.find((o) => o.key === sortConfig.key)?.label ?? "Sort By";
  ...
  if (loading) return <p style={{ padding: "2rem" }}>Loading {title}...</p>;
  ...
  return (
    <div className="module active">
      <div className="module-header flex justify-between items-center">
        ...
        {selectedIds.size > 0 ? (
          ...
          <button
            ...
            onClick={() => router.push("/maintenance/doctype/device-type/new")}
          >
      ...
      <div className="search-filter-section flex items-center gap-3 mt-4">
        <input
          type="text"
          ...
          {isSortMenuOpen && (
            <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow border z-50">
              {SORT_OPTIONS.map((opt) => (
                ...
                >
                  {opt.label} {sortConfig.key === opt.key && <Check className="w-4 h-4" />}
                </button>
              ))}
            </div>
          )}
